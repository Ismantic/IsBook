...
后面的七篇文章会沿着这两条主线逐层展开：前四篇更接近 Python 运行时和程序表示，后三篇进入编译、代码生成与动态执行。
...
Matx 是一个实验性的 Python 子集编译器与运行时项目，不以实现完整 Python 为目标。本书关注已经连通的普通函数编译主线，以及支撑这条主线所需的 Python 语言机制；尚不完整的解释器、类、闭包和模块导入能力只会在相关位置说明边界，不把设计目标当成现有实现。
