...
这一章先不急着谈 NLP 模型，而是先回答一个更基本的问题：文本在机器里到底是什么。

很多实现上的问题，看上去发生在分词、tokenizer 或检索阶段，根源却常常更早。比如：

- 以为“一个字符”总能对应一个字节
- 把字符串长度和字节长度混为一谈
- 没有意识到 UTF-8 是一种编码，而不是字符集本身

所以这一章要先把文本表示的底层逻辑讲清楚。
...
理解完概念之后，再进入实现部分。这里的目标不是写一个“功能尽可能全”的库，而是通过几个小函数把 UTF-8 编解码真正拆开来看。
