...
正则表达式很容易被误解成一组零碎技巧：什么时候用 `*`，什么时候用 `?`，什么时候加转义。

但如果把视角拉远一点，它其实是在回答一个更本质的问题：如何用一套紧凑的语法，去描述一个字符串集合。

也正因为如此，正则表达式同时拥有两个面向：

- 它在工程上很好用，是最常见的文本规则工具之一
- 它在理论上也很清楚，对应着正则语言和有限状态自动机

这一章先从这个角度进入，再落到一个最小 matcher 的实现。
