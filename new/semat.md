...
**LDA 的生成过程：**
- **主题分布**：对每个主题 \\(t\\)，从狄利克雷先验中采样词语分布 \\(\phi_t \sim \operatorname{Dirichlet}(\beta)\\)
...
需要维护起来，否则每次重新求和仍然要遍历全部主题。缓存本身不会必然损失精度：每次把一个词语从旧主题移动到新主题时，只有这两个主题的总计数发生变化，因此可以先减去它们原来的贡献，再加入更新后的贡献，以 \\(O(1)\\) 的代价精确更新 \\(S_s\\)。
...
这并不意味着整个算法已经完全无锁。所有线程仍然会更新全局主题计数 \\(n_{\cdot|t}\\)。实现时可以使用原子操作，也可以维护线程局部增量并定期合并；如果直接使用未同步的陈旧计数，则得到的是近似并行采样，需要明确说明一致性与性能之间的取舍。
...
## Perplexity
困惑度（Perplexity）用于衡量模型对未见文档的预测能力。通常应当在留出的验证集或测试集上计算，而不是直接使用训练语料上的当前主题分配。
...
其中总词数为测试集中所有文档的词语数量。困惑度可以直观理解为模型在每个位置面对的平均有效候选数；在相同数据集和评估方法下，数值越低，表示模型的预测能力越好。实际评估 LDA 时，还需要为测试文档推断文档—主题分布，并避免使用待评估词语本身的信息。
