...
深度学习的训练过程，本质上是根据损失函数对参数的梯度不断调整参数。一个训练引擎因此必须解决两个问题：如何记录前向计算，以及如何沿着计算过程反向传播梯度。

Zero 使用反向模式自动微分。它在前向计算时动态构建计算图，在调用 `Backward()` 后，再按照与前向相反的顺序遍历这张图。
...
1. `Add` 向两个输入传递梯度 \\(1\\)。
2. `Mul` 对 \\(x\\) 产生梯度 \\(y=2\\)，对 \\(y\\) 产生梯度 \\(x=3\\)。
3. \\(x\\) 同时经过 `Mul` 和 `Add` 到达 \\(z\\)，因此需要累加两条路径。
